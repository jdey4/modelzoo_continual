...
IMG_SIZE = 28
speakers = {"g":0, "j":1, "l":2, "n":3, "t":4, "y":5}
DATA_PATH = '/Users/jayantadey/free-spoken-digit-dataset/recordings/'
#%%
def load_spoken_digit(path_recordings):
    ...
    for i in file:
        ...
        x_stft_db = librosa.amplitude_to_db(
            abs(x_stft)
        )  # Convert an amplitude spectrogram to dB-scaled spectrogram
        x_stft_db_mini = cv2.resize(x_stft_db, (IMG_SIZE, IMG_SIZE))  # Resize into 28 by 28
        y_n = i[0]  # number
        ...
        y_number.append(int(y_n) + speakers[y_s]*10)
...
tmap, cmap = {}, {}
for tid, task in enumerate(spoken):
    for lid, lab in enumerate(task):
        tmap[lab], cmap[lab] = tid, lid

data_x, data_y = load_spoken_digit(DATA_PATH)

#%%
def get_data(task=0, seed=0):
    train_X = []
    train_y = []
    test_X = []
    test_y = []

    indx = np.where((data_y>=task*NUM_CLASS_PER_TASK) & (data_y<(task+1)*NUM_CLASS_PER_TASK))[0]

    X, y = data_x[indx], data_y[indx]
    (
        train_X,
        test_X,
        train_y,
        test_y,
        ) = train_test_split(X, y, test_size=0.45, random_state=seed)
    
    train_X = np.array(train_X).reshape(-1,1,IMG_SIZE,IMG_SIZE)
    train_y = np.array(train_y)
    test_X = np.array(test_X).reshape(-1,1,IMG_SIZE,IMG_SIZE)
    test_y = np.array(test_y)
    
    return train_X, train_y, test_X, test_y

def create_spoken_task(tasks, train=True, shuffle=False, bs=256, seed=0):
    ...
    for task_id in tasks:
         ...
         get_data(task=task_id, seed=seed)
...
class SmallConv(nn.Module):
    ...
    def __init__(self, channels=3, avg_pool=2, lin_size=320):
        ...
        self.fc3 = nn.Linear(in_features=2000, out_features=NUM_CLASS_PER_TASK*EPISODES)
...
def train_model(tasks, epochs):
    ...
    mttask_loader = create_spoken_task(tasks, shuffle=True, bs=32)
...
def evaluate_zoo(episode, zoo_outputs):
    ...
    return met
# %%
def run_zoo(bb=5, epochs=50, seed=0):
    zoo_outputs = {}
    zoo_log = {}
    train_losses = []
    ...
    '''df_singletask['task'] = list(range(1,7))
    df_singletask['accuracy'] = list(zoo_log[ep]['test_acc'])

    with open('spoken_digit/model_zoo-'+str(seed)+'.pickle', 'wb') as f:
        pickle.dump(df_singletask, f)'''

    with open('spoken_digit/model_zoo-'+str(seed)+'.pickle', 'rb') as f:
...
for i in range(EPISODES):
    tr, te = create_spoken_task([i], train=False, seed=seed_)
...
for seed_ in range(0,10):
    zoo_log = run_zoo(bb=5, epochs=EPISODES, seed=seed_)
    #isolated_log = run_zoo(bb=1, epochs=EPISODES, seed=seed_)
# %%
